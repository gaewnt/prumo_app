...
import {
  ...
  fetchDetoxLogsForMonth,
  countOnDate,
  type DetoxHabitInput,
} from "@/lib/detox";

/** Conteúdo de Detox — usado tanto na rota própria quanto como aba dentro do hub Desenvolvimento Pessoal. */
export function DetoxContent() {
  ...
  // Histórico de meses anteriores — `logs` só cobre os últimos 7 dias (a
  // busca de sempre do Detox), então aqui busca sempre o mês selecionado, inclusive o atual.
  // Um único seletor de mês compartilhado entre todos os hábitos, pra não repetir a navegação
  // em cada card.
  const now = new Date();
  const [historyMonth, setHistoryMonth] = useState(new Date(now.getFullYear(), now.getMonth(), 1));
  const historyLogsQuery = useQuery({
    queryKey: ["detox", "monthLogs", userId, historyMonth.getFullYear(), historyMonth.getMonth()],
    queryFn: () => fetchDetoxLogsForMonth(historyMonth),
    enabled: !!userId,
  });
  const heatmapLogs = historyLogsQuery.data ?? [];
    ...
    <View style={{ gap: 20 }}>
      ...
      {query.isLoading ? (
        ...
        <View style={{ gap: 10 }}>
          ...
          {habits.length > 0 ? <MonthNav monthDate={historyMonth} onChange={setHistoryMonth} /> : null}
          ...
          {habits.map((habit) => (
            <DetoxHabitCard
              ...
              getHistoryCellColor={(dateStr) => {
                const count = countOnDate(heatmapLogs, habit.id, dateStr);
                return count > 0 ? tokens.accentMuted : null;
              }}
