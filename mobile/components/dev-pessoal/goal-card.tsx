...
import { deriveGoalStatus, daysUntil, type Goal } from "@/lib/dev-pessoal";
...
type GoalCardProps = {
  ...
  onStartEdit: () => void;
  onCancelEdit: () => void;
  onUpdate: (input: { title: string; description: string; targetDate: string | null }) => void;
  isSaving: boolean;
...
export function GoalCard({
  ...
  const remainingDays = goal.target_date && status === "active" ? daysUntil(goal.target_date) : null;
  ...
  return (
    ...
    >
      <Pressable onPress={onToggleCompleted} disabled={isTogglingCompleted} hitSlop={8}>
        <View
          style={{
            width: 24,
            height: 24,
